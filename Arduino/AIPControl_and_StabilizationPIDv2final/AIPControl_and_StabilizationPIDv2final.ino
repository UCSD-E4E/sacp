...
void loop(){
  ...
  //commands are: 'u' = pitch BUMPed up, 'd' = pitch BUMPed down, 
  //'p'N = pitch to N degrees , where N is float between 0-180.0,
  //'f' = activate filter, 'n' = deactivate filter 's' = activate stabilization
  //'z' = deactivate stabilization, 'y'N = add N degrees to current yaw 
  //'x' = bump yaw right 10 degrees, 'y' = bump yaw left 10 degrees
  switch (command)
  {
  case 'x':
    if((yawCenter + 10) >= 360)
  ...
  case 'u':
    if((mappedPitchCenter + BUMP) <= 165.0)
    { 
  ...
  case 'd':
    if((mappedPitchCenter - BUMP) >= 10.0)
    {
      mappedPitchCenter = mappedPitchCenter - BUMP;
    }
    break;
  ...
  case 'p':
    ...
    if ( (newPitch >= 30.0) && (newPitch <= 140.0))
    {
  ...
  if(UM6_Packet.Address == UM6_REG_EULER_ROLL_PITCH) {
   ...
   Serial.println(yawCenter);
    ...
    if(activateStabilize)
    {
       ...
       //yaw stabilization
       yawServo.writeMicroseconds(yawProp());
...
//yawProp simple proportion controller for yaw
int yawProp()
{
  int proportion;
  ...
  {
    proportion = YAW_FLAT - ((yawCenter - yaw) * YAW_PROP_CONSTANT);
    #ifdef DEBUG2
  ...
  {
    proportion = YAW_FLAT - (((yawCenter + 360) - yaw) * YAW_PROP_CONSTANT);
    #ifdef DEBUG2
  ...
  {
    proportion = YAW_FLAT + ((yaw - yawCenter) * YAW_PROP_CONSTANT);
    #ifdef DEBUG2
  ...
  {
    proportion = YAW_FLAT + (((yaw + 360) - yawCenter) * YAW_PROP_CONSTANT);
    #ifdef DEBUG2
...
int rollPID()
{
  ...
  if(sumRoll > MAX_SUM_ROLL) 
  {
    sumRoll = MAX_SUM_ROLL;
  }
    #ifdef DEBUG
...
int pitchPID()
{
  ...
  if(sumPitch > MAX_SUM_PITCH)
  {
    sumPitch = MAX_SUM_PITCH;
  }
  #ifdef DEBUG
